...
def get_aug(p=0.5):
    return A.Compose([
        ...
        A.OneOf([
         ...
         A.GaussianBlur(p=p,blur_limit=(5,7)),
        ]),
        # # # ##A.GlassBlur(p=p,sigma=0.1), (x)
        ...
        # Dilation 
        # no dislation and erosion in albumentations 
    ])
...
class ABINetRecAug(object):
    def __init__(self,
                 ...
                 colorjitter_p=0.25,
                 aug_function = get_aug(p=0.5),
                 **kwargs):
        self.transforms_a = aug_function
    ...
    def __call__(self, data):
        img = data['image']
        aug = self.transforms_a(image=img)
        #------
        img = aug['image']
        #------        
