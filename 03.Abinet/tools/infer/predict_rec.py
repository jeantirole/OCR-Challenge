        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            for i in range(len(preds)):
                ...
                for j in range(len(preds[i])):
                    max_conf = np.max(preds[i][j])
                    if np.where(preds[i][j] == max_conf) != (np.array([0]),):
                        conf_list_tmp.append(max_conf)
                conf_list[indices[beg_img_no + i]] = conf_list_tmp
            for rno in range(len(rec_result)):
                rec_res[indices[beg_img_no + rno]] = rec_result[rno]
            if self.benchmark:
                self.autolog.times.end(stamp=True)
        return rec_res, conf_list, time.time() - st
...
def main(args):
    ...
    for image_file in image_file_list:
        ...
        img_list.append(img)
    try:
        rec_res, conf_list, _ = text_recognizer(img_list)
    ...
    f = open(f"{args.rec_model_dir}/submission_conf.csv", "w", newline='')
    f2 = open(f"{args.rec_model_dir}/submission.csv", "w", newline='')
    wr = csv.writer(f)
    wr.writerow(['id', 'label', 'confidence_score'])
    wr2 = csv.writer(f2)
    wr2.writerow(['id', 'label'])

    for ino in range(len(img_list)):
        logger.info("Predicts of {}:{}".format(valid_image_file_list[ino], rec_res[ino]))
        wr.writerow([os.path.basename(valid_image_file_list[ino]).split('.')[0], rec_res[ino][0], conf_list[ino]])
        wr2.writerow([os.path.basename(valid_image_file_list[ino]).split('.')[0], rec_res[ino][0]])
